...
// Vercel provides env at runtime; no need to load .env here
...
// MongoDB connection (reuse existing connection across invocations)
...
const Vault = mongoose.models.Vault || mongoose.model('Vault', {
...
app.post('/api/vault', authenticateToken, async (req, res) => {
  try {
    ...
    const vaultItem = new Vault({ userId: req.user.userId, encryptedData });
...
app.put('/api/vault/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const { id } = req.params;
    if (!encryptedData) return res.status(400).json({ error: 'Encrypted data is required' });
    const vaultItem = await Vault.findOneAndUpdate({ _id: id, userId: req.user.userId }, { encryptedData }, { new: true });
